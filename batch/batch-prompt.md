...
2. ATS-optimized customized PDF
3. Tracker line for subsequent merging
...
**RULE: For article metrics, article-digest.md overrides cv.md.** cv.md might have older numbers — this is normal.
**RULE: Before evaluating, load `modes/_profile.md` and `config/profile.yml` if they exist.** They contain candidate preferences AND concrete scoring rules that **overwrite** system defaults.

Pattern types these files might include:
- **Block caps** — e.g., "cap Block A at 3.0/5 if title contains 'Lead'/'Head'/'Principal'"
- **Recommendation overrides** — e.g., "force SKIP if comp ceiling below $120K"
...
- **Block F:** apply recommendation overrides (forced SKIP, etc.) — `_profile.md` can convert a technically high score into a SKIP by shape or comp

**In conflict, rules in `_profile.md` win over `_shared.md` defaults.** This is intentional: `_profile.md` is the user customization layer.
...
2. If the file is empty or does not exist, try to get JD from `{{URL}}` using WebFetch
...
> **Concrete metrics are read from `cv.md` + `article-digest.md` per evaluation. NEVER hardcode numbers here.**

| If the role is... | Emphasize about the candidate... | Proof point sources |
|-----------------|--------------------------|--------------------------|
| Platform / LLMOps | System builder in production, observability, evals, closed-loop | article-digest.md + cv.md |
...
**Transversal advantage**: Frame profile as **"Technical builder"** that adapts framing to the role:
...
Turn "builder" into a professional signal, not a "hobby maker". The framing changes, the truth remains.
...
Read `cv.md`. Table with each JD requirement mapped to exact lines of the CV or i18n.ts keys.
...
Where `{company-slug}` is the company name in lowercase, no spaces, dashed.
...
| `{{EXPERIENCE}}` | Experience HTML with reordered bullets |
| `{{SECTION_PROJECTS}}` | Projects |
| `{{PROJECTS}}` | Top 3-4 projects HTML |
...
```
{next_num}\t{{DATE}}\t{company}\t{role}\t{status}\t{score}/5\t{pdf_emoji}\t[{{REPORT_NUM}}](reports/{{REPORT_NUM}}-{company-slug}-{{DATE}}.md)\t{note_1_phrase}
```
...
**IMPORTANT:** TSV order has status BEFORE score (col 5→status, col 6→score). In applications.md the order is inverted (col 5→score, col 6→status). merge-tracker.mjs handles the conversion.
